"""Gap - Working Around Gaps (D+J): Ability to explain around missing vocabulary without losing meaning."""
...
def score_gap(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Gap", name="Working Around Gaps", category="Language Quality",
    ...
    prompt = f"""Rate how well this interview candidate works around vocabulary gaps: ability to explain around missing vocabulary without losing meaning.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Gap", name="Working Around Gaps", category="Language Quality", layer="D+J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python gap.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_gap(report)
