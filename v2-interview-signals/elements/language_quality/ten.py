"""Ten - Tense (M+J): Tense errors relative to intended meaning."""
...
def score_ten(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Ten", name="Tense", category="Language Quality",
    ...
    prompt = f"""Rate the tense accuracy of this interview candidate (higher = better).
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Ten", name="Tense", category="Language Quality", layer="M+J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ten.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_ten(report)
