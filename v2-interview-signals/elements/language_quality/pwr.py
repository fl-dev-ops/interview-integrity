"""Pwr - Power Words (J): Strength, precision, and active quality of word choice."""
...
def score_pwr(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Pwr", name="Power Words", category="Language Quality",
                            layer="J", source="transcript", score=0, confidence=0.3,
                            raw={}, evidence=["No turns"])

    text = "\n".join(f"[{i+1}] {t['text'][:400]}" for i, t in enumerate(turns[:8]))

    prompt = f"""Rate the power word usage of this interview candidate: strength, precision, and active quality of word choice.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Pwr", name="Power Words", category="Language Quality", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python pwr.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_pwr(report)
