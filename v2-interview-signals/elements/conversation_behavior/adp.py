"""Adp - Adaptability (J): Adjustment to follow-up questions, corrections, or new direction."""
...
def score_adp(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Adp", name="Adaptability", category="Conversation Behavior",
    ...
    prompt = f"""Rate the adaptability of this interview candidate: adjustment to follow-up questions, corrections, or new direction.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Adp", name="Adaptability", category="Conversation Behavior", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python adp.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_adp(report)
