"""Ans - Responsive (J): Directness of response to prompt."""
...
def score_ans(report: dict) -> SignalResult:
    qa_pairs = build_qa_pairs(report)
    if not qa_pairs:
        return SignalResult(code="Ans", name="Responsive", category="Conversation Behavior",
    ...
    prompt = f"""Rate the responsiveness of this interview candidate: directness of response to prompt.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Ans", name="Responsive", category="Conversation Behavior", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ans.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_ans(report)
