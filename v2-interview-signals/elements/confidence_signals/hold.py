"""Hold - Pressure Hold (D+J): Stability when question difficulty increases."""
...
def score_hold(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Hold", name="Pressure Hold", category="Confidence Signals",
    ...
    prompt = f"""Rate the pressure hold of this interview candidate: stability when question difficulty increases.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Hold", name="Pressure Hold", category="Confidence Signals", layer="D+J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python hold.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_hold(report)
