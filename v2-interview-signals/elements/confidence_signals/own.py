"""Own - Ownership (J): Active agency and responsibility in answers."""
...
def score_own(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Own", name="Ownership", category="Confidence Signals",
    ...
    prompt = f"""Rate the ownership of this interview candidate: active agency and responsibility in answers.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Own", name="Ownership", category="Confidence Signals", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python own.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_own(report)
