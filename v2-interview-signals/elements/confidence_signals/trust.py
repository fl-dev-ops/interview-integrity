"""Trust - Trust (J): Whether the listener would trust the candidate based on answer quality and delivery."""
...
def score_trust(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Trust", name="Trust", category="Confidence Signals",
    ...
    prompt = f"""Rate the trustworthiness of this interview candidate: whether the listener would trust the candidate based on answer quality and delivery.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Trust", name="Trust", category="Confidence Signals", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python trust.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_trust(report)
