"""Ex - Example (J): Strength and relevance of chosen example."""
...
def score_ex(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Ex", name="Example", category="Answer Structure",
    ...
    prompt = f"""Rate the example quality of this interview candidate: strength and relevance of chosen example.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Ex", name="Example", category="Answer Structure", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ex.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_ex(report)
