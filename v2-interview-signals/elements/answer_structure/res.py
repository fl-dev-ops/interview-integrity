"""Res - Result (J): Outcome after the candidate's action."""
...
def score_res(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Res", name="Result", category="Answer Structure",
    ...
    prompt = f"""Rate the result quality of this interview candidate: outcome after the candidate's action.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Res", name="Result", category="Answer Structure", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python res.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_res(report)
