"""Step - Steps (J): Sequence of actions/process."""
...
def score_step(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Step", name="Steps", category="Specificity",
    ...
    prompt = f"""Rate the step quality of this interview candidate: sequence of actions/process.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Step", name="Steps", category="Specificity", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python step.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_step(report)
