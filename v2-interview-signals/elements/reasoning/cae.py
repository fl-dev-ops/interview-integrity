"""C&E - Cause/Effect (J): Causal link between action and result."""
...
def score_cae(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="C&E", name="Cause/Effect", category="Reasoning",
    ...
    prompt = f"""Rate the cause/effect reasoning of this interview candidate: causal link between action and result.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="C&E", name="Cause/Effect", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python cae.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_cae(report)
