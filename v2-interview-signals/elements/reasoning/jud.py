"""Jud - Judgment (J): Deliberate professional choice and ownership of consequence."""
...
def score_jud(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Jud", name="Judgment", category="Reasoning",
    ...
    prompt = f"""Rate the judgment of this interview candidate: deliberate professional choice and ownership of consequence.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Jud", name="Judgment", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python jud.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_jud(report)
