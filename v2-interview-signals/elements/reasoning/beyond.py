"""Beyond - Goes Beyond (J): Lesson, system, follow-up action, or broader principle beyond the direct answer."""
...
def score_beyond(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Beyond", name="Goes Beyond", category="Reasoning",
    ...
    prompt = f"""Rate how well this interview candidate goes beyond the direct answer: lesson, system, follow-up action, or broader principle.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Beyond", name="Goes Beyond", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python beyond.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_beyond(report)
