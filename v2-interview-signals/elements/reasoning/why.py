"""Why - Reasoning (J): Explanation of why a choice/action was made."""
...
def score_why(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Why", name="Reasoning", category="Reasoning",
    ...
    prompt = f"""Rate the reasoning quality of this interview candidate: explanation of why a choice/action was made.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Why", name="Reasoning", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python why.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_why(report)
