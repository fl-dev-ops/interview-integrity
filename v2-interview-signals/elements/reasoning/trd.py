"""Trd - Tradeoffs (J): Awareness of what was gained/lost in a decision."""
...
def score_trd(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Trd", name="Tradeoffs", category="Reasoning",
    ...
    prompt = f"""Rate the tradeoff awareness of this interview candidate: awareness of what was gained/lost in a decision.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Trd", name="Tradeoffs", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python trd.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_trd(report)
