"""Ref - Reflection (J): What changed afterward because of the experience."""
...
def score_ref(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Ref", name="Reflection", category="Reasoning",
    ...
    prompt = f"""Rate the reflection quality of this interview candidate: what changed afterward because of the experience.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Ref", name="Reflection", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ref.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_ref(report)
