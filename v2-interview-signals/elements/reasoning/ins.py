"""Ins - Insight (J): New understanding produced by the experience."""
...
def score_ins(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Ins", name="Insight", category="Reasoning",
    ...
    prompt = f"""Rate the insight quality of this interview candidate: new understanding produced by the experience.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Ins", name="Insight", category="Reasoning", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ins.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_ins(report)
