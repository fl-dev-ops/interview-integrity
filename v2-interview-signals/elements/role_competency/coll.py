"""Coll - Collab (J): Teamwork and collaboration evidence."""
...
def score_coll(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Coll", name="Collab", category="Role Competency",
    ...
    prompt = f"""Rate the collaboration of this interview candidate: teamwork and collaboration evidence.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Coll", name="Collab", category="Role Competency", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python coll.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_coll(report)
