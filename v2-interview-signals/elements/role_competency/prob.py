"""Prob - Problem Solve (J): Ability to handle ambiguity, constraints, or issues."""
...
def score_prob(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Prob", name="Problem Solve", category="Role Competency",
    ...
    prompt = f"""Rate the problem solving of this interview candidate: ability to handle ambiguity, constraints, or issues.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Prob", name="Problem Solve", category="Role Competency", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python prob.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_prob(report)
