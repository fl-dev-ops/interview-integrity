"""Comp - Competency (C): Whether the answer demonstrates behavior needed for the role."""
...
def score_comp(report: dict, prior_signals: PriorSignals = None) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Comp", name="Competency", category="Role Competency",
    ...
    signal_ctx = build_signal_context(prior_signals, ["Rel", "Ex", "Act", "Jud", "Imp"])

    prompt = f"""Rate the competency of this interview candidate: whether the answer demonstrates behavior needed for the role.
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python comp.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_comp(report)
