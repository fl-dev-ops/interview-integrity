"""Lead - Leadership (J): Initiative, influence, or guidance of others."""
...
def score_lead(report: dict) -> SignalResult:
    turns = get_candidate_turns(report)
    if not turns:
        return SignalResult(code="Lead", name="Leadership", category="Role Competency",
    ...
    prompt = f"""Rate the leadership of this interview candidate: initiative, influence, or guidance of others.
...
Return JSON: {{"score": <0-4>, "reasoning": "<brief>"}}"""
    ...
    return SignalResult(
        code="Lead", name="Leadership", category="Role Competency", layer="J",
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python lead.py <report.json>"); sys.exit(1)
    report = json.loads(Path(sys.argv[1]).read_text())
    result = score_lead(report)
